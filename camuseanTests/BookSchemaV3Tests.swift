...
// V3 schema coverage. A lightweight migration stage has no mapping closure to unit-test, so the
// risk it carries is a malformed schema/relationship that only blows up at container init or on
// delete. These tests build a real in-memory V3 container and prove: the schema is valid, the
...
@MainActor
@Suite struct BookSchemaV3Tests {

    private func makeV3Context() throws -> ModelContext {
    ...
    // The V3 container opens at all (the schema + relationship are well-formed) and a Book with
    // its future-ready identity fields round-trips.
    @Test func v3ContainerOpensAndBookRoundTrips() throws {
        let context = try makeV3Context()
        let book = Book(
            title: "L'Étranger",
            author: "Albert Camus",
            language: "fr-FR",
